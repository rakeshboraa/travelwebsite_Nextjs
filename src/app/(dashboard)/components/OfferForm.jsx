...
const Page = ({ isEditMode }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            let url = '/api/combo-offers';
            let method = 'POST';
            if (isEditMode) {
                url = `/api/combo-offers/${formData._id}`;
                method = 'PUT';
            }
            ...
            if (response.ok) {
                setSuccessMessage(isEditMode ? 'Combo offer updated successfully!' : 'Combo offer saved successfully!');
                setMessageType('success');
                setFormData({
                    _id: '',
                    name: '',
        ...
        <div className="grid mx-[40px] flex-1 auto-rows-max gap-4">
            <div className="grid gap-4 md:grid-cols-[1fr_250px] lg:grid-cols-3 lg:gap-8">
                <div className="grid auto-rows-max items-start gap-4 lg:col-span-2 lg:gap-8">
                    <Card x-chunk="dashboard-07-chunk-2">
                        <CardHeader>
                            <CardTitle>{isEditMode ? 'Edit Combo Offer' : 'Add Product for Offers'}</CardTitle>
                        </CardHeader>
                        <CardContent>
            ...
            <div className="flex items-center justify-center gap-2">
                ...
                <Button size="sm" onClick={handleSubmit}>
                    {isEditMode ? 'Update Combo Offer' : 'Save Combo Offer'}
                </Button>
            </div>
        </div>
    );
